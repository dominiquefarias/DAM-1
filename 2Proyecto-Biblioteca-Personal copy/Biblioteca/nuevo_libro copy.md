Para comenzar conecté el archivo de llamado config que lo utilizamos para poder conectarnos con la base de datos y utilizo uno que se llama mysqli_report para que reporte los errores en la conexion con la base de datos lo cual es muy importante para detectar fallos de sql durante el funcionamiento de la `pagina tambien lo que hago es declarar variables que me ayudan a identificar si estoy en modo edicion o no y si estoy editando un libro o no. Asi como tambien diseñamos una sección inicial la cual detecta automáticamente si hay un parámetro ?edit en la URL Ya que lo encuentra cambio el modo de la página a edición la cualk consulta a la base de datos para traer toda la información existente de ese libro incluyendo autor, editorial y mi valoración personal y preparo las variables para pre-llenar el formulario más adelante el cual es el siguiente
```php
<?php

include "includes/config.php";

// Reportar errores en la conexion con la base de datos
...
// Si el usuario quiere editar un libro
if (isset($_GET['edit'])) {
    $id_edit = intval($_GET['edit']);
    
    // Consulta para rellenar el formulario el cual me traera toda la informacion del libro que quiera editar el usuario
    $sql_edit = "SELECT L.*, 
...
```
Esta sección comienza tipo a "funcionar" cuando el usuario presione el boton de "Guardar" se inicia una transacción entre la base de datos para asegurar que todas las operaciones se guarden correctamente por eso mismo se utiliza begin_transaction(), asi como tambien por eso se pone todos los datos que aparecen en la base de datos para guardarlos estos en variables y que puedan ser utilizados en el formulario, recoje todos los datos enviados por el usuario también hemos incluido un pequeño bloque el cual normaliza el "estado" del libro luiego de eso se asegura de que lo que viene del formulario coincida exactamente con los valores que hemos predeterminado en la base de datos
```php
// Si el usuario quiere guardar los datos del libro
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        $conexion->begin_transaction();

        // priemro recogemos los datso con el metodo POST que sirve para recoger los datos que vienen del formulario
        $titulo = $_POST['titulo'];
        ...
        // luego corregimos el estado del libro para que coincida con los valores que hemos predeterminado en la base de datos
        $estado_raw = $_POST['estado']; 
        $estado_db = 'Deseado'; 
        if ($estado_raw === 'leyendo') $estado_db = 'Leyendo';
        if ($estado_raw === 'leido')   $estado_db = 'Leido'; 
```
Para la portada del libro hemos creado un sistema en el cual hemos utilizado el metodo POST para que verifique si el usuario subió una nueva imagen si es así genera un nombre único usando la fecha actual para evitar que se dupliquen las imágenes ya que esta se encuentra en la carpeta uploads/ si no existe y mueve el archivo allí Si no se sube nada pero estoy editando mantiene la ruta de la portada antigua para no perderla. Tambien hemos creado un sistema el cual nos permite obtener el id del autor, editorial y categoria del libro que se encuentra en la base de datos para que pueda ser guardado en la base de datos
```php
        // Si la imagen es nueva o se esta editando
        ...
        // Imagen nueva o editar y guardar la imagen asi como tambien la ruta de la imagen
...
```
Para mantener la base de datos "limpoia y ordenada" utilizamos la función getIdByName lo que hace es inteligente: primero busca si el autor (o editorial) ya existe. Si existe, me devuelve su ID. Si no existe, lo crea automáticamente y me devuelve el ID del nuevo registro. Esto me permite escribir nombres libremente en el formulario sin preocuparme por gestionar IDs manualmente.
```php
        // Relaciones (Autor/Editorial/Categoria)
